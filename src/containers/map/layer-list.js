...
import { setObjectsDataFilter, setShowOffices, setShowHomeAddress, setDomainsFilter } from '../../ducks/map';
...
class LayerList extends Component {
    ...
    handleBankChange = (value) => {
        this.setState((state) => ({
            bankValue: value,
            dataFilter: Map(state.dataFilter).set('department', value === 'all' ? null : `(department == "${value}")`).toJS()
        }), () => {this.setFilter()})
    };
    ...
    handleSelectLiquidity = (value) => {
        this.setState((state) => ({
            ...
            dataFilter: Map(state.dataFilter).set('liquidity', value === 'all' ? null : `(actual_liquidity == {null} && liquidity == "${value}" || actual_liquidity == "${value}")`).toJS()
        }), () => {this.setFilter()});
    };
    ...
    handleEnsure2Select = (value) => {
        ...
        if(value.length === 1) CLASSIFIER[value[0]].forEach(item => toEnsure3.push({value: item, text: item}));

        this.setState(state => ({
            ensureType2Value: value,
            disableEnsure3: value.length > 1,
            ensureType3Items: toEnsure3,
            dataFilter: Map(state.dataFilter).set('classifier2', value.length === 5 ? null : '(' + value.map(e => `classifier2 == "${e}"`).join(' || ') + ')').toJS()
        }), () => {this.setFilter()})
    };
    ...
    handleEnsure3Select = (value) => {
        this.setState(state => ({
            ...
            dataFilter: Map(state.dataFilter).set('classifier3', value === 'all' ? null : `(classifier3 == "${value}")`).toJS()
        }), () => {this.setFilter()})
    };
    ...
    resetForm = () => {
        this.setState(LayerListInitState.toJS(), () => {this.setFilter()});
    };
    ...
    componentWillMount () {
        ...
        attr.forEach(item => {
            if(item.name === 'department'){
                item.domain && this.setState(state => ({bankItems: [...state.bankItems, ...item.domain.map(el => ({value: el, text: el}))]}))
            }
            if(item.name === 'object_quality_category'){
                item.domain && this.setState(state => ({qualityCategoryItems: [...state.qualityCategoryItems, ...item.domain.map(el => ({value: el, text: el}))]}))
            }
            if(item.name === 'liquidity'){
                item.domain && this.setState(state => ({liquidityItems: [...state.liquidityItems, ...item.domain.map(el => ({value: el, text: el}))]}))
            }
        })
    };
        ...
        return (
            <MapControl onCollapse={this.handleCollapse} style={layerListStyle} collapsed={collapsed}>
                <CSSTransitionGroup
                    ...
                    {collapsed && <div className="extend-filter">
                        ...
                        <div className="form-control">
                            <span className="label">Плановые даты проверки:</span>
                            <DateInput libProps={{
                        ...
                        <div className="form-control">
                            <span className="label">Категория качества:</span>
                        ...
                        <div className="form-control">
                            ...
                            <Select style={{width: '100%'}} value={liquidityValue} items={liquidityItems} onChange={this.handleSelectLiquidity} />
                        </div>
                        <div className="form-control">
                            <span className="label">Вид обеспечения по классификатору (2 уровень):</span>
                            <Select selectItemFormatter={this.ensure2ItemFormatter} multi style={{width: '100%'}} value={ensureType2Value} items={ensureType2Items} onChange={this.handleEnsure2Select} />
                        </div>
                        <div className="form-control">
                            <span className="label">Вид обеспечения по классификатору (3 уровень):</span>
                            <Select disable={disableEnsure3} style={{width: '100%'}} value={ensureType3Value} items={ensureType3Items} onChange={this.handleEnsure3Select} />
                        </div>
                        <div style={{textAlign: 'center', position: 'relative', zIndex: 0}}>
                            <RoundedButton onClick={this.resetForm} label="Сбросить фильтр" />
                        </div>
                    </div>}
                </CSSTransitionGroup>
                <LayersList
