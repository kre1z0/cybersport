...
class SelectFieldInput extends Component {
    ...
    handleSelectItem = (event, child) => {
        ...
        if (multiple) {
            ...
            if (onChange)
                onChange(data.filter(item => arr.indexOf(item.id) > -1));
        } else {
            this.handleBlur();
            if (onChange)
                onChange(data.filter(item => item.id === child.props.id)[0]);
        }
    };
    ...
    render() {
        ...
        return (
            <div className={mergedClassName}>
                <div
                    ref={ref => {
                        this.selectBoxRef = ref;
                    }}
                    style={style}
                    onClick={this.handleClick}
                    className={classNames('select-box', { focused })}
                >
